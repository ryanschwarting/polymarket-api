...
// Define the response type for markets by category
...
export async function GET() {
  try {
    ...
    // Fetch markets from the Gamma API with active=true and closed=false query parameters
    const response = await fetch(`${GAMMA_API_URL}?active=true&closed=false`, {
      method: "GET",
      headers: {
    ...
    const markets: RawMarketData[] = await response.json();

    console.log(`Gamma API response received with ${markets.length} markets`);
    ...
    console.log(
      `Successfully fetched ${markets.length} markets from Gamma API`
    );
    ...
    return Response.json({
      ...
      message: "All markets sorted by volume",
      totalMarkets: formattedMarkets.length,
    });
  } catch (error) {
    console.error("Error in markets-by-category API route:", error);
